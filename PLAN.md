...
| **HIGH** | [#57 Commercial-friendly TTS expansion](#57-commercial-friendly-tts-backend-expansion) | Phased | Phases 1-3 DONE; Turbo WORKING; F0 wired in; native voice cloning shipped → HISTORY §82; **#83 production fix LANDED 2026-05-24 → HISTORY 2026-05-24 + LEARNINGS Round 9** — S3Gen UNet weight residency split (`s3.fd.*` on CPU, encoder/vocoder GPU): M1 Metal cos_min 0.940→**0.999980** in diff harness, intelligible audio at all T; comparable wall-time to pure CPU on M1. Q8_0×F32 bit-match Metal kernel committed (commit `752baecf`, upstream-PR-quality, drafted as PR 09). 3 upstream PR drafts in `tools/upstream-prs/09-11` covering Metal Q8_0 kernel + ggml-alloc drift bug report + scheduler NaN-at-large-T bug report. Linux CPU smoke validated on VPS. **FIXED** 2026-05-24 (R9 follow-up #4): two independent bugs in tandem. **Bug 1** (ggml sched): `ggml_backend_sched_split_graph` mutates `gf->nodes[i]->src[j]` to point at internal tensors in `sched->ctx`, which is freed at the start of the NEXT split_graph call — leaving dangling pointers. Repeated `alloc_graph + graph_compute` on the same gf (like CFG's cond+uncond passes) loses input copy detection on the second call. Patched with a per-call mutation log in `ggml/src/ggml-backend.cpp` that restores src[j] originals at end of compute. **Bug 2** (residual): even with bug 1 fixed, `unet_input` specifically needs to live on the Metal backend; sched-copy-on-every-call delivers correct bytes but downstream compute still diverges. Sidestepped by `ggml_backend_sched_set_tensor_backend(sched, unet_input, c->backend)`. Bisect: pinning `time_emb` is HARMFUL (forces mish onto GPU, rms ~209). Both fixes are required. M1 Metal diff `s3gen_mel cos_min 0.940 → 0.999976` (parity with production CPU residency), smoke `rms 13.938 → 5.143`, 2-mark trigger NaN → 5.291. Production CPU-residency path unchanged. Also open: Kartoffelbox_Turbo DE |
