...
inline std::string lts_word_to_ipa(const std::string& word) {
    ...
    for (size_t i = 0; i < word.size(); i++) {
        ...
        // Lowercase accented (C3 xx range)
            ...
            if (c2 >= 0x80 && c2 <= 0x9E) { w += (char)c; w += (char)(c2 + 0x20); i++; }
            else { w += word[i]; }
        }
        else w += word[i];
    }
    ...
    auto prev_vowel = [&](int pos) -> bool {
        return pos > 0 && is_vowel(w[pos-1]);
    };
    ...
    for (int i = 0; i < len;) {
        ...
        if ((unsigned char)c == 0xC3 && i + 1 < len) {
            ...
            // ñ = C3 B1
            if (c2b == 0xB1) { ipa += "\xC9\xB2"; i += 2; continue; } // ɲ
            // á = C3 A1
            if (c2b == 0xA1) { ipa += "a"; i += 2; continue; }
            // é = C3 A9
            if (c2b == 0xA9) { ipa += "e"; i += 2; continue; }
            // í = C3 AD
            if (c2b == 0xAD) { ipa += "i"; i += 2; continue; }
            // ó = C3 B3
            if (c2b == 0xB3) { ipa += "o"; i += 2; continue; }
            // ú = C3 BA
            if (c2b == 0xBA) { ipa += "u"; i += 2; continue; }
            // ü = C3 BC
            if (c2b == 0xBC) { ipa += "w"; i += 2; continue; }
            i += 2; continue; // skip unknown accented
        }

        // --- Multi-char ---
        if (c == 'c' && c1 == 'h') { ipa += "t\xCA\x83"; i += 2; continue; }  // ch → tʃ
        if (c == 'l' && c1 == 'l') { ipa += "\xCA\x9D"; i += 2; continue; }    // ll → ʝ (yeísmo)
        if (c == 'r' && c1 == 'r') { ipa += "r"; i += 2; continue; }            // rr → r (trill)
        if (c == 'q' && c1 == 'u') { ipa += "k"; i += 2; continue; }
        if (c == 'g' && c1 == 'u') {
            // gu before e/i: silent u
            if (c2 == 'e' || c2 == 'i') {
                if (is_stop_context(i)) ipa += "\xC9\xA1"; // ɡ
                else ipa += "\xC9\xA3"; // ɣ
                i += 2; continue;
            }
        }
        ...
        if (c == 'b' || c == 'v') {
            ...
            if (is_stop_context(i)) ipa += "b";
            else ipa += "\xCE\xB2"; // β
            i++; continue;
        }
        if (c == 'c') {
            if (c1 == 'e' || c1 == 'i') { ipa += "s"; i++; continue; } // seseo
            ipa += "k"; i++; continue;
        }
        if (c == 'd') {
            if (is_stop_context(i)) ipa += "d";
            else ipa += "\xC3\xB0"; // ð
            i++; continue;
        }
        if (c == 'e') { ipa += "e"; i++; continue; }
        if (c == 'f') { ipa += "f"; i++; continue; }
        if (c == 'g') {
            if (c1 == 'e' || c1 == 'i') { ipa += "x"; i++; continue; } // jota
            if (is_stop_context(i)) ipa += "\xC9\xA1"; // ɡ
            else ipa += "\xC9\xA3"; // ɣ
            i++; continue;
        }
        ...
        if (c == 'j') { ipa += "x"; i++; continue; } // jota
        if (c == 'k') { ipa += "k"; i++; continue; }
        if (c == 'l') { ipa += "l"; i++; continue; }
        if (c == 'm') { ipa += "m"; i++; continue; }
        if (c == 'n') { ipa += "n"; i++; continue; }
        if (c == 'o') { ipa += "o"; i++; continue; }
        if (c == 'p') { ipa += "p"; i++; continue; }
        if (c == 'r') {
            if (i == 0) ipa += "r"; // trill word-initial
            else ipa += "\xC9\xBE"; // ɾ tap
            i++; continue;
        }
        if (c == 's') { ipa += "s"; i++; continue; }
        if (c == 't') { ipa += "t"; i++; continue; }
        if (c == 'u') { ipa += "u"; i++; continue; }
        if (c == 'w') { ipa += "w"; i++; continue; }
        if (c == 'x') { ipa += "ks"; i++; continue; }
        if (c == 'y') {
            if (i == len - 1) ipa += "i"; // word-final y → vowel
            else ipa += "\xCA\x9D"; // ʝ
            i++; continue;
        }
        if (c == 'z') { ipa += "s"; i++; continue; } // seseo
        i++; // skip unknown
    }
    return ipa;
}
